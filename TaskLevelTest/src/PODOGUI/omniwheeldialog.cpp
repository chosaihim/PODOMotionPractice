...
void OmniWheelDialog::on_OW_GOTO_DES_clicked()
{
    ...
    des_v = str_des_v.toDouble();

    if((bSuccess1 == false)||(bSuccess2 == false)||(bSuccess3 == false)) return;
...
void OmniWheelDialog::on_PB_WHEEL_UP_clicked()
{
    USER_COMMAND cmd;
    cmd.COMMAND_TARGET = AlnumOmniWheel;
    cmd.COMMAND_DATA.USER_COMMAND = OMNIWHEEL_AL_CHANGEPOS;
    cmd.COMMAND_DATA.USER_PARA_CHAR[0] = 10;
    pLAN->SendCommand(cmd);
}

void OmniWheelDialog::on_PB_WHEEL_DOWN_clicked()
{
    USER_COMMAND cmd;
    cmd.COMMAND_TARGET = AlnumOmniWheel;
    cmd.COMMAND_DATA.USER_COMMAND = OMNIWHEEL_AL_CHANGEPOS;
    cmd.COMMAND_DATA.USER_PARA_CHAR[0] = 11;
    pLAN->SendCommand(cmd);
}
