    ...
    ui(new Ui::GUIMainWindow)
{
    ...
    dialogWalking = new WalkingDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogWalking, "Walking");

    dialogWheel = new OmniWheelDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogWheel, "Wheel");

    dialogJoystick = new JoyStickDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogJoystick, "Joystick");

    dialogLiftBox = new LiftBoxDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogLiftBox, "Box");

    dialogZMPGraph = new ZMPGraphDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogZMPGraph, "ZMP");

    dialogDoor = new DoorDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogDoor, "Door");

    dialogInverse = new InverseCheckDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogInverse, "Inv");

    dialogWBWalk = new WBWalkDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogWBWalk, "WBWalk");

    dialogRealWalk = new RealWalkDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogRealWalk, "RWalk");

    dialogTestDialog = new TestDialog(this);
    ui->MAIN_TAB->addTab((QWidget*)dialogTestDialog, "Test");
...
void GUIMainWindow::SLOT_NEW_DATA(){
    ...
    // Power ======================
    if(PODO_DATA.CoreData.SP[0].Voltage < 48.0){
        err += "Voltage Low ";
    }
    if(PODO_DATA.CoreData.SP[0].Current > 8.0){
        err += "Current High ";
    }

    ui->LB_VOLTAGE->setText(QString().sprintf("%.1fV", PODO_DATA.CoreData.SP[0].Voltage));
    ui->LB_CURRENT->setText(QString().sprintf("%.1fA", PODO_DATA.CoreData.SP[0].Current));
    // ============================

    // MC Stat ====================
    for(int i=0; i<NO_OF_JOINTS; i++){
        mSTAT ms = PODO_DATA.CoreData.MCStatus[MC_GetID(i)][MC_GetCH(i)];
    ...
    if(PODO_DATA.CoreData.REF_Enabled)   ui->LE_REF_STAT->setStyleSheet("background-color: green");
    else                                ui->LE_REF_STAT->setStyleSheet("background-color: red");

    if(PODO_DATA.CoreData.ENC_Enabled)   ui->LE_ENC_STAT->setStyleSheet("background-color: green");
    else                                ui->LE_ENC_STAT->setStyleSheet("background-color: red");

    if(PODO_DATA.CoreData.SEN_Enabled)   ui->LE_SEN_STAT->setStyleSheet("background-color: green");
    else                                ui->LE_SEN_STAT->setStyleSheet("background-color: red");

    if(PODO_DATA.CoreData.CAN_Enabled)   ui->LE_CAN_STAT->setStyleSheet("background-color: green");
...
//void GUIMainWindow::on_BTN_SAVE_DEBUG_clicked()
//{
//    USER_COMMAND cmd;
