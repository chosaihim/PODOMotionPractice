...
void RBCore_RTThreadCon(void *)
{
    ...
    while(IS_WORKING)
    {
        ...
        if(canHandler->IsWorking()){
            ...
            sharedData->IMU[0].Roll_offset = _DEV_IMU[0].ROLL_OFFSET;
            sharedData->IMU[0].Pitch_offset = _DEV_IMU[0].PITCH_OFFSET;
        }
        // ===========================================================
...
void RBCMD_SensorIMUOffsetSet(){
    ...
    settings.setValue("roll", QString().sprintf("%.4f", roll));
    settings.setValue("pitch", QString().sprintf("%.4f", pitch));
}
...
void RBCMD_SensorIMU_Zero(){
    int id = 0;//sharedData->COMMAND[RBCORE_PODO_NO].USER_PARA_CHAR[0];

    float R2D = 57.2957802f;
    float D2R = 0.0174533f;
//std::cout <<"HAHAHAHAH! done!" << endl;
//    if(id == -1){   //All
//        for(int i=0; i<_NO_OF_IMU; i++){
//            std::cout <<"HAHAHAHAH! done!" << endl;
//            _DEV_IMU[i].ROLL = _DEV_IMU[i].ACC_X;
//            _DEV_IMU[i].PITCH = _DEV_IMU[i].ACC_Y;
//            _DEV_IMU[i].YAW = 0.0;//

//            _DEV_IMU[i].Quat_IMU0 = 0.5*sqrt(1.0 + cos(_DEV_IMU[i].PITCH*D2R)*cos(_DEV_IMU[i].YAW*D2R) + sin(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].PITCH*D2R)*sin(_DEV_IMU[i].YAW*D2R) + cos(_DEV_IMU[i].ROLL*D2R)*cos(_DEV_IMU[i].YAW*D2R) + cos(_DEV_IMU[i].ROLL*D2R)*cos(_DEV_IMU[i].PITCH*D2R) );

//            _DEV_IMU[i].Quat_IMU1 = (sin(_DEV_IMU[i].ROLL*D2R)*cos(_DEV_IMU[i].PITCH*D2R) - (cos(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].PITCH*D2R)*sin(_DEV_IMU[i].YAW*D2R) - sin(_DEV_IMU[i].ROLL*D2R)*cos(_DEV_IMU[i].YAW*D2R)))/4.0;

//            _DEV_IMU[i].Quat_IMU2 = (cos(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].PITCH*D2R)*cos(_DEV_IMU[i].YAW*D2R) + sin(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].YAW*D2R) - (-sin(_DEV_IMU[i].PITCH*D2R)))/4.0;

//            _DEV_IMU[i].Quat_IMU3 = ((cos(_DEV_IMU[i].PITCH*D2R)*sin(_DEV_IMU[i].YAW*D2R)) - (sin(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].PITCH*D2R)*cos(_DEV_IMU[i].YAW*D2R) - cos(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].YAW*D2R)))/4.0;

//        }
//    }else{  //Each
//    _DEV_IMU[0].ROLL_OFFSET  = -_DEV_IMU[0].IMU_ROLL_NULL     + (_DEV_IMU[0].ACC_X);
//    _DEV_IMU[0].PITCH_OFFSET = -_DEV_IMU[0].IMU_PITCH_NULL    + (_DEV_IMU[0].ACC_Y);
//    _DEV_IMU[0].YAW_OFFSET   = -_DEV_IMU[0].IMU_YAW_NULL      + (0.0);

        _DEV_IMU[id].ROLL = _DEV_IMU[id].ACC_X;
        _DEV_IMU[id].PITCH = _DEV_IMU[id].ACC_Y;
        _DEV_IMU[id].YAW = 0.0;//

        _DEV_IMU[id].Quat_IMU0 = 0.5*sqrt(1.0 + cos(_DEV_IMU[id].PITCH*D2R)*cos(_DEV_IMU[id].YAW*D2R) + sin(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].PITCH*D2R)*sin(_DEV_IMU[id].YAW*D2R) + cos(_DEV_IMU[id].ROLL*D2R)*cos(_DEV_IMU[id].YAW*D2R) + cos(_DEV_IMU[id].ROLL*D2R)*cos(_DEV_IMU[id].PITCH*D2R) );

        _DEV_IMU[id].Quat_IMU1 = (sin(_DEV_IMU[id].ROLL*D2R)*cos(_DEV_IMU[id].PITCH*D2R) - (cos(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].PITCH*D2R)*sin(_DEV_IMU[id].YAW*D2R) - sin(_DEV_IMU[id].ROLL*D2R)*cos(_DEV_IMU[id].YAW*D2R)))/4.0;

        _DEV_IMU[id].Quat_IMU2 = (cos(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].PITCH*D2R)*cos(_DEV_IMU[id].YAW*D2R) + sin(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].YAW*D2R) - (-sin(_DEV_IMU[id].PITCH*D2R)))/4.0;

        _DEV_IMU[id].Quat_IMU3 = ((cos(_DEV_IMU[id].PITCH*D2R)*sin(_DEV_IMU[id].YAW*D2R)) - (sin(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].PITCH*D2R)*cos(_DEV_IMU[id].YAW*D2R) - cos(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].YAW*D2R)))/4.0;
...
//    }
    ...
    sharedData->FOG.Roll = sharedData->IMU[0].Roll;
    sharedData->FOG.Yaw = sharedData->IMU[0].Yaw;
    sharedData->FOG.PitchVel = sharedData->IMU[0].PitchVel;
    sharedData->FOG.RollVel = sharedData->IMU[0].RollVel;
    sharedData->FOG.YawVel = sharedData->IMU[0].YawVel;
