...
int RBProcessManager::OpenAL(int no){
    ...
    sharedData->COMMAND[no].USER_COMMAND = NO_ACT;
    pid_t tempPID = fork();
    if(tempPID == 0){
        pidAL[no] = tempPID;
...
void RBProcessManager::CloseAL(int no){
    ...
    for(int i=0;i<_NO_OF_MC;i++){
        for(int j=0;j<_DEV_MC[i].MOTOR_CHANNEL;j++){
            if(sharedData->MotionOwner[i][j] == no){
                _DEV_MC[i].MoveJoints[j].RefAngleCurrent = sharedData->ENCODER[i][j].CurrentReference;
...
int RBProcessManager::CheckALWorking(int no){
    UpdatePID(no);
    if(pidAL[no] <= 0){
        sharedData->PODO_AL_WORKING[no] = false;
        return false;
    }
    else{
        if(CheckIsZombie(pidAL[no])){
            ...
            sharedData->PODO_AL_WORKING[no] = true;
            return true;
        }
    }
}
