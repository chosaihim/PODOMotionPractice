...
void ROSThread::run(){
    ...
    if(__IS_GAZEBO){
        ...
        worker.timePrev = worker.timeCur = sharedData->Sim_Time_sec + sharedData->Sim_Time_nsec/1000000000.0;
...
void ROSWorker::onNewConnection(){
    printf("ROS");
    timePrev = timeCur = sharedData->Sim_Time_sec + sharedData->Sim_Time_nsec/1000000000.0;
}
...
void ROSWorker::onFastTimer(){
    ...
    // send data to ROS
    timeCur = sharedData->Sim_Time_sec + sharedData->Sim_Time_nsec/1000000000.0;
...
void ROSWorker::SendtoROS(){
    for(int i=0; i<NO_OF_JOINTS; i++){
        TXData.JointReference[i] = sharedData->ENCODER[MC_GetID(i)][MC_GetCH(i)].CurrentReference;
        TXData.JointEncoder[i] = sharedData->ENCODER[MC_GetID(i)][MC_GetCH(i)].CurrentPosition;
    }
