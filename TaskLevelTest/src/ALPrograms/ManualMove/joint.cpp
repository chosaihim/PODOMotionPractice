...
int JointControlClass::RefreshToCurrentEncoder(){
    if(Shm->ENCODER[MC_ID_CH_Pairs[RKN].id][MC_ID_CH_Pairs[RKN].ch].CurrentPosition < 0.0001 && Shm->ENCODER[MC_ID_CH_Pairs[RKN].id][MC_ID_CH_Pairs[RKN].ch].CurrentPosition > -0.0001){
        ...
        for(int i=LSP; i<=LWP; i++){
            ...
            Joints[i]->SetRefAngleCurrent(Shm->ENCODER[mcId][mcCh].CurrentPosition);
            Shm->JointReference[PODONum][mcId][mcCh] = Joints[i]->GetRefAngleCurrent();
        }
        ...
        Joints[LWY2]->SetRefAngleCurrent(Shm->ENCODER[mcId][mcCh].CurrentPosition);
        Shm->JointReference[PODONum][mcId][mcCh] = Joints[LWY2]->GetRefAngleCurrent();
        ...
        Joints[RWY2]->SetRefAngleCurrent(Shm->ENCODER[mcId][mcCh].CurrentPosition);
        Shm->JointReference[PODONum][mcId][mcCh] = Joints[RWY2]->GetRefAngleCurrent();
...
int JointControlClass::RefreshToCurrentEncoder_RAP_RF1(){
    if(fabs(Shm->ENCODER[MC_ID_CH_Pairs[RKN].id][MC_ID_CH_Pairs[RKN].ch].CurrentPosition) < 0.00001){
        return 0; //Encoder is off
    }
    else{
        int mcId, mcCh;
        ...
        Joints[RAP]->SetRefAngleCurrent(Shm->ENCODER[mcId][mcCh].CurrentPosition);
        Shm->JointReference[PODONum][mcId][mcCh] = Joints[RAP]->GetRefAngleCurrent();
