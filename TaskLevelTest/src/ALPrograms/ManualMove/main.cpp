    ...
    while(isTerminated == 0){
        ...
        case ManualMove_AL_UPPER_TASK_POSE:
            ...
            if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0] == 0){
                ...
                // HP movement
                double des_HP = sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[0];
                joint->SetMoveJoint(RHP, des_HP, 3000, MOVE_RELATIVE);
                joint->SetMoveJoint(LHP, des_HP, 3000, MOVE_RELATIVE);
                usleep(3100*1000);
            }

            sharedData->COMMAND[PODO_NO].USER_COMMAND = ManualMove_AL_NO_ACT;
            break;
        case ManualMove_AL_HAND:
            if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0]==0)// right hand
            {
                double des_cur = 125.*((double)(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1]));
        ...
        case ManualMove_AL_GAIN:
            if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0]==0)//right arm
            {
                if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1]==0)//low gain
                {
...
//                    RBJointGainOverride(3,20,1000,1000,1000);//LWY LWP
        ...
        case ManualMove_AL_MANUAL_BOTH_FOOT_MODE_START:
            printf("Manual Both Hand with wheel Start...!!\n");
        ...
        case ManualMove_AL_DRIVE_MODE:
            if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[0] == 0){ //start
...
//                RBJointGainOverride(2, 36,0,1000,1);
            ...
            else if(sharedData->COMMAND[PODO_NO].USER_PARA_INT[0] == 1){ //finish
...
void RBTaskThread(void *)
{
    ...
    while(isTerminated == 0){
        ...
        else if(WheelModeFlag == true) {

            ManualMoveHandwithwheel();

            ManualMoveWheel();//Yujin
            CalculateMovingEverage();

            joint->SetMoveJoint(RWH, RWHnow, 5, MOVE_ABSOLUTE);
            joint->SetMoveJoint(LWH, LWHnow, 5, MOVE_ABSOLUTE);
        }
        else if(WheelModeFlag_both == true) {

            ManualMoveHandwithwheel_Right();
            ManualMoveHandwithwheel_Left();
...
void ShutDownManualMode(void){
    ...
    WheelModeFlag = false;
    WheelModeFlag_both = false;
    //---------------------------------
    usleep(1000);

    for(int i=0;i<10;i++){
        sharedData->COMMAND[PODO_NO].USER_PARA_INT[i] = 0;
    }
    for(int i=0;i<4;i++){
        sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[i] = 0;
    }
}
...
{
    while(sharedData->COMMAND[RBCORE_PODO_NO].USER_COMMAND != NO_ACT);

    sharedData->COMMAND[RBCORE_PODO_NO].USER_PARA_CHAR[0] = 1;
//    sharedData->COMMAND[RBCORE_PODO_NO].USER_PARA_CHAR[1] = on;

    sharedData->COMMAND[RBCORE_PODO_NO].USER_COMMAND = DAEMON_SENSOR_ENCODER_ONOFF;
    cout<<"Enc_request On"<<endl;
}
// --------------------------------------------------------------------------------------------- //
void ManualMoveWheel(void)// when we use joystick
{
    double radius = 0.0;
    double angularV = 0.0;
    ...
    int Joy_RJOG_UD = GL_JOY_RJOG_UD/2;

    // Stop case----------------------------------------------------
    if(Joy_RJOG_UD == 0){//yujin
    ...
    if(GL_JOY_LJOG_RL==0)
    {
        ...
        lin_vel = ((double)Joy_RJOG_UD)/65000.0;//50:1
...
//        if(GL_JOY_LJOG_RL == 0){
            ...
            printf("RJOG_UD = %d, RWHList = %d, LWHList = %d \n", Joy_RJOG_UD, RWHList, LWHList);
    ...
    else
    {
        ...
        if(GL_JOY_LJOG_RL<0){
            vel1 = fabs(((double)Joy_RJOG_UD)/regul_mag);
        }else{
            vel1 = -fabs(((double)Joy_RJOG_UD)/regul_mag);
        }
        vel2 =-vel1;
...
void ManualMoveHandwithwheel(void) {
...
//    static double RH_SpeedUP_V0[6] = {0,0,0};
 ...
 //   double ShoulderToHand;
    ...
    static int grasping_count_plus = 0;
    static int grasping_count_minus = 0;
...
//===================initialize variables, joints to current reference==============
        if(RL_mode == RIGHT){
            for(int i=0 ;i<3 ; i++){
...
//            RH_SpeedUP_V0[i] = pVel[i];
 ...
 //         RH_SpeedUP_V0[i] = AngSpeed[i];
    ...
    if(Joy_Y_before==(int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1] && Joy_A_before==(int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[2] && Joy_X_before==(int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0]
            && Joy_B_before==(int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[3] && Joy_RT_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[4] && Joy_RB_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[5]
            && Joy_LT_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[6] && Joy_LB_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[7] && Joy_AROW_RL_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[8]
            && Joy_AROW_UD_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[9] && Joy_RJOG_RL_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[0] && Joy_LJOG_RL_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[1]
            && Joy_RJOG_UD_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[2] && Joy_LJOG_UD_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[3] && Joy_log_before ==sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[14])
    {
        same_counter++;
    }
    else{
    ...
    if(Joy_LJOG_UD > -32767 && Joy_LJOG_UD <= 0 && plus_Dcc_time[6] > 0 && Joy_LJOG_PUSH == 0){
        MoveFlag[6] = false;
        if(RL_mode == RIGHT)
            RELB += AngSpeed[6]*R2D*0.2;
        else
    ...
    if(Joy_LJOG_UD < 32767 && Joy_LJOG_UD >=0 && minus_Dcc_time[6] > 0 && Joy_LJOG_PUSH == 0){
        MoveFlag[6] = false;
        if(RL_mode == RIGHT)
            RELB -= AngSpeed[6]*R2D*0.2;
        else
    ...
    // Finger Grasping//yujin
    if(Joy_log== 1 && log_flag == 0){
                Grasping_value = -125/2;
                grasping_count_plus++;
            }
            else if(Joy_log==0 && grasping_count_plus > 0){
                Grasping_value = 0;
                grasping_count_plus=0;
                log_flag=1;
            }
            else if(Joy_log == 1 && log_flag == 1){
                Grasping_value = 125/2;
                grasping_count_minus++;
                cout << grasping_count_minus << endl;
            }
            else if(Joy_log ==0 && grasping_count_minus > 0){
                grasping_count_minus=0;
                Grasping_value = 0;
                log_flag=0;
            }
            else{
    ...
    if(Two_Hand_switching_counter >= 50){
            ...
            cout<<"OK>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<endl;
            //Two_Hand_switching_counter = 0;
            sharedData->COMMAND[PODO_NO].USER_COMMAND = ManualMove_AL_MANUAL_BOTH_FOOT_MODE_START;
...
void ManualMoveHandwithwheel_Right(void){

    //cout << "ManualMoveHand with wheel right" << endl;
...
//            RH_SpeedUP_V0[i] = pVel[i];
 ...
 //         RH_SpeedUP_V0[i] = AngSpeed[i];
    ...
    if((Joy_BACK == 1 && Joy_START == 0)||(Joy_BACK == 0 && Joy_START == 1)){
    ...
    if(One_Hand_switching_counter >= 40){

            cout<<"OK<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"<<endl;
            sharedData->COMMAND[PODO_NO].USER_COMMAND = ManualMove_AL_MANUAL_FOOT_MODE_START;
            //One_Hand_switching_counter = 0;
...
void ManualMoveHandwithwheel_Left(void) {
    double pVel[3] = {0,0,0};
//    static double RH_SpeedUP_V0[6] = {0,0,0};
    static double SpeedDW_V0[8] = {0,0,0,0,0,0,0,0};
    static int SpeedUp_time[8] = {0,0,0,0,0,0,0,0};
...
//            RH_SpeedUP_V0[i] = pVel[i];
 ...
 //         RH_SpeedUP_V0[i] = AngSpeed[i];
    ...
    if(Joy_Y_before==(int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1] && Joy_A_before==(int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[2] && Joy_X_before==(int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0]
            ...
            && Joy_LT_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[6] && Joy_LB_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[7] && Joy_AROW_RL_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[8]
            && Joy_AROW_UD_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[9] && Joy_RJOG_RL_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[0] && Joy_LJOG_RL_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[1]
            && Joy_RJOG_UD_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[2] && Joy_LJOG_UD_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[3])
    {
        same_counter++;
    }
    else{
...
//    static bool Release_Flag = false;
    ...
    // Joystick data
//    int Joy_Y = (int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1];
//    int Joy_A = (int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[2];
//    int Joy_X = (int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0];
//    int Joy_B = (int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[3];
//    int Joy_RT = sharedData->COMMAND[PODO_NO].USER_PARA_INT[4];
//    int Joy_RB = sharedData->COMMAND[PODO_NO].USER_PARA_INT[5];
//    int Joy_LT = sharedData->COMMAND[PODO_NO].USER_PARA_INT[6];
//    int Joy_LB = sharedData->COMMAND[PODO_NO].USER_PARA_INT[7];
//    int Joy_AROW_RL = sharedData->COMMAND[PODO_NO].USER_PARA_INT[8];
//    int Joy_AROW_UD = sharedData->COMMAND[PODO_NO].USER_PARA_INT[9];
//    int Joy_RJOG_RL = sharedData->COMMAND[PODO_NO].USER_PARA_INT[0];
//    int Joy_LJOG_RL = sharedData->COMMAND[PODO_NO].USER_PARA_INT[1];
//    int Joy_RJOG_UD = sharedData->COMMAND[PODO_NO].USER_PARA_INT[2];
//    int Joy_LJOG_UD = sharedData->COMMAND[PODO_NO].USER_PARA_INT[3];
//    int Joy_BACK = sharedData->COMMAND[PODO_NO].USER_PARA_INT[10];
//    int Joy_START = sharedData->COMMAND[PODO_NO].USER_PARA_INT[11];
//    int Joy_RJOG_PUSH = sharedData->COMMAND[PODO_NO].USER_PARA_INT[12];
//    int Joy_LJOG_PUSH = sharedData->COMMAND[PODO_NO].USER_PARA_INT[13];
    int Joy_Y = GL_JOY_Y;
    int Joy_A = GL_JOY_A;
    int Joy_X = GL_JOY_X;
    int Joy_B = GL_JOY_B;
...
//            RH_SpeedUP_V0[i] = pVel[i];
 ...
 //   if(counter%100 == 0)
 //       cout<<"Joy Value: "<<sharedData->COMMAND[PODO_NO].USER_PARA_INT[1]<<" time: "<<SpeedUp_time<<" pVel: "<<pVel<<" MoveFlag: "<<MoveFlag<<endl;
...
//    }
 ...
 //       One_Hand_switching_counter = 0;
    ...
    if(Two_Hand_switching_counter >= 50){
        if(WB_FLAG_WHOLEBODY == true){
            cout<<"OK>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<endl;
            sharedData->COMMAND[PODO_NO].USER_COMMAND = ManualMove_AL_MANUAL_BOTH_HAND_STADING_START;
            //Two_Hand_switching_counter = 0;
        }
        else{
            cout<<"OK>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"<<endl;
            //Two_Hand_switching_counter = 0;
            sharedData->COMMAND[PODO_NO].USER_COMMAND = ManualMove_AL_MANUAL_BOTH_HAND_MODE_START;
...
//        sharedData->COMMAND[PODO_NO].USER_COMMAND = ManualMove_AL_MANUAL_MODE_START;
//        One_Hand_switching_counter = 0;
//    }
...
//    }
    if(Two_Foot_switching_counter >= 20){
        ...
        if(WB_FLAG_WHOLEBODY == true){
            cout<<"OK<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"<<endl;
            sharedData->COMMAND[PODO_NO].USER_COMMAND = ManualMove_AL_MANUAL_ONE_HAND_STADING_START;
            //One_Hand_switching_counter = 0;
        }
        else{
            cout<<"OK<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"<<endl;
...
//    int Joy_A = (int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[2];
    ...
    if(Joy_Y_before==(int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1] && Joy_A_before==(int)sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[2]
            && Joy_AROW_RL_before==sharedData->COMMAND[PODO_NO].USER_PARA_INT[8])
    {
        same_counter++;
    }
    else{
...
//        RBJointOLCurrentCommand2ch(0, 4, 30, 0, 0x05);
    ...
    if(Joy_Y == 0){
        if(sharedData->ENCODER[MC_ID_CH_Pairs[RAP].id][MC_ID_CH_Pairs[RAP].ch].CurrentPosition < 18 ){
...
//        cout<<sharedData->COMMAND[PODO_NO].USER_PARA_INT[8]<<endl;
