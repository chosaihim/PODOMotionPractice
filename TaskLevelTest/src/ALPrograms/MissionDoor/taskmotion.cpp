...
void TaskMotion::WBIK(){
    ...
    static double COM_InitPos[3] = {0.,}, COM_Local[3] = {0.,};
    static int cnt1 = 0;
    static int cnt2 = 0;

    if(OnOff_zmpcontrol == true)
    {
        if(cnt1 == 0)
        {
            COM_InitPos[0] =  des_pCOM_2x1[0];
            COM_InitPos[1] =  des_pCOM_2x1[1];
        }
        des_pCOM_2x1[0] = COM_InitPos[0];
        des_pCOM_2x1[1] = COM_InitPos[1];
        //printf("COM_LocalX = %f , cnt = %d\n",des_pCOM_2x1[0],cnt1);
        cnt1++;
    } else
    {
        cnt1 = 0;
    ...
    if(FLAG_leaned == true)
    {
        if(cnt2 == 0)
        {
            printf("Save Init COM pos...\n");
            COM_InitPos[0] =  des_pCOM_2x1[0];
            COM_InitPos[1] =  des_pCOM_2x1[1];
        }
        COM_Local[0] = des_pCOM_2x1[0] - COM_InitPos[0];
        COM_Local[1] = des_pCOM_2x1[1] - COM_InitPos[1];
        printf("COM_Local = %f\n",COM_Local[0]);
        cnt2++;
    } else
    {
    ...
    des_pCOM_2x1[0] += OnOff_zmpcontrol*ZMPControlX;
    des_pCOM_2x1[1] += OnOff_zmpcontrol*ZMPControlY;

    des_pRH_3x1[0] += OnOff_compliance*PosX[0];// + FLAG_leaned*COM_Local[0];
    des_pRH_3x1[1] += OnOff_compliance*PosY[0];
    des_pRH_3x1[2] += OnOff_compliance*PosZ[0];

    des_pLH_3x1[0] += OnOff_compliance*PosX[1];// + FLAG_leaned*COM_Local[0];
    des_pLH_3x1[1] += OnOff_compliance*PosY[1];
    des_pLH_3x1[2] += OnOff_compliance*PosZ[1];



    //des_pRH_3x1[0] += FLAG_leaned*COM_Local[0];
...
void TaskMotion::WBIK_PARA_CHANGE(){
        // Tuned value with the head
    kine_drc.C_Torso[0] = 0.000941-0.065;
    // weight of Torso is increased ( Torso + Head)
    kine_drc.m_Torso = 24.98723 + 0.0; // kg
    kine_drc.m_RightWrist = 4.5;
    kine_drc.m_LeftWrist = 4.5;
    kine_drc.m_Pelvis = 11.867886 +2.;
    kine_drc.C_Torso[1] =0;
    kine_drc.L_FOOT = 0.113;
    kine_drc.iter_limit = 100;
    kine_drc.converge_criterium = 1e-6;
