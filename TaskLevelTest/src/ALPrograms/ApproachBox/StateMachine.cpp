...
void update_window()
{
    ...
    if(target_foot[0].time.dsp_time + target_foot[0].time.ssp_time < cnt_2_time(window_cnt))
    {
        ...
        {// Move the target_foot sequence one space forward and fill last target_foot from short_foot[]
            ...
            printf("mode is %d\n",new_footprint.mode);
            if(new_footprint.mode == WALKING_CHANGE)
            {
                FLAG_BacktoNormalWalk++;
                printf("FLAG_Back is %d\n",FLAG_BacktoNormalWalk);
            }
            memcpy(&last_short_foot, &new_footprint, sizeof(_footprint_info));
    ...
    if(target_foot[0].mode == WALKING_SINGLELOG || target_foot[0].mode == WALKING_CHANGE)
    {
        update_window_foot_ref_singlelog(window_cnt);
    } else
    {
