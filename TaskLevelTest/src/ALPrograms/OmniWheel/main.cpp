...
//        FILE_LOG(logINFO) << odom_x << ", " << odom_y;
        ...
        case OMNIWHEEL_AL_GOTODES:
            ...
            _des_x=sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[0];
            _des_y=sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[1];
            _des_a=sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[2];

            // NAN CHECK
            if(isnan(_des_x) || isnan(_des_y) || isnan(_des_a)){
                sharedData->COMMAND[PODO_NO].USER_COMMAND = OMNIWHEEL_AL_NO_ACT;
                break;
                printf("----------!!!---- Is NAN OUT----!!!----------\n");
            }
            ...
            if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[3] == 3){
                velChangeFlag = true;
                sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[3] =0;
            }else{
                velChangeFlag = false;
            }
            if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[4] == 4){
                noWaitFlag = true;
                sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[4] =0;
            }else{
                noWaitFlag = false;
            }
            ...
            if(velChangeFlag == true){
                velChangeFlag = false;
                _max_wheel_rot = sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[3];
            ...
            if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[5] == 5){
                joint->RefreshToCurrentReference();
                joint->SetAllMotionOwnerWHEELandWST();
                usleep(5*1000);
                sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[5]=0;
                Tank_Mode_Flag = true;
            ...
            if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[6] == 6){
                directCompenFlag = true;
                sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[6] =0;
            }else{
                directCompenFlag = false;
            }
            directCompenFlag = true;
        ...
        case OMNIWHEEL_AL_RADIUS:
            ...
            {
            WMR_dir=sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0];//1 is right, 0 is left
            WMR_fb=sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1];//1 is go foward -1 is go backward
            WMR_radius=sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[0];
            WMR_angle=sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[1];
            ...
            if(radius_velChangeFlag == true){
                radius_velChangeFlag = false;
                WM_radius_vel = sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[3];
            }else
                WM_radius_vel = 150.;
        ...
        case OMNIWHEEL_AL_VELMODE:
            ...
            else if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0]==1)
            {
                ...
                double OW_des_vel=sharedData->COMMAND[PODO_NO].USER_PARA_DOUBLE[0];
        ...
        case OMNIWHEEL_AL_MANUAL:
            if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0]==1)//manual move on
            {
                joint->RefreshToCurrentReference();
                joint->SetAllMotionOwnerWHEEL();
            ...
            else if(sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[0]==0)//manual move off
            {
        ...
        case OMNIWHEEL_AL_CONTROL:
            ...
            {
                char onoff = sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1];
                if(onoff ==1){
                    REAL_MODE = true;
...
//                    usleep((temp_ms+10)*1000);
            ...
            {
                char mode = sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1];
                WB_FLAG = false;
                usleep(10*1000);
            ...
            {
                char mode = sharedData->COMMAND[PODO_NO].USER_PARA_CHAR[1];
                float del_ang = sharedData->COMMAND[PODO_NO].USER_PARA_FLOAT[0];
                WB_FLAG = false;
                usleep(10*1000);
                if(mode == 1){//up
                    // gain return
...
void RBTaskThread(void *)
{
     while(isTerminated == 0)
    {
         ...
         double vr = sharedData->ENCODER[MC_GetID(RWH)][MC_GetCH(RWH)].CurrentVelocity*D2Rf;
         double vl = sharedData->ENCODER[MC_GetID(LWH)][MC_GetCH(LWH)].CurrentVelocity*D2Rf;
...
//         double w_r = 0.051;
         ...
         double dx = (sl+sr)/2.0*cos(sharedData->FOG.Yaw*D2Rf + sharedData->FOG.YawVel*dt/2.0);
         double dy = (sl+sr)/2.0*sin(sharedData->FOG.Yaw*D2Rf + sharedData->FOG.YawVel*dt/2.0);
         double dtheta = sharedData->FOG.YawVel*dt;

         odom_x += dx;
         odom_y += dy;
         odom_theta = sharedData->FOG.Yaw*D2Rf;

         double w = sharedData->FOG.YawVel;
...
//            FILE_LOG(logINFO) << sharedData->vel_cmd[0] << ", " << sharedData->vel_cmd[1];
        ...
        case WHEEL_MANUALMODE:
            ...
            L_JOG_RL=sharedData->COMMAND[PODO_NO].USER_PARA_INT[0];
            AROW_RL=sharedData->COMMAND[PODO_NO].USER_PARA_INT[1];
            R_JOG_UD=sharedData->COMMAND[PODO_NO].USER_PARA_INT[2];
        ...
        if(FLAG_WHEELup == true)
        {
            WHEELup();
        }
        if(FLAG_WHEELdown == true)
        {
            WHEELdown();
        }
        ...
        // Yaw compensation
        _OW_YAW_VALUE += sharedData->FOG.YawVel*R2D*0.005;
        //printf("FOG yaw = %f\n",sharedData->FOG.Yaw);
    ...
    case MOTION_2:
        ...
        else
        {
            ...
            if(ulCount==gap_end_T3)
            {
                ...
                double input = sharedData->FOG.YawVel*R2D*0.005;
...
int RBInitialize(void)
{
    ...
    if(rt_task_create(&rtFlagCon, "OmniWheel_FLAG", 0, 95, 0) == 0){
        cpu_set_t aCPU;
        CPU_ZERO(&aCPU);
        CPU_SET(3, &aCPU);
    ...
    if(rt_task_create(&rtTaskCon, "OmniWheel_TASK", 0, 90, 0) == 0){
        cpu_set_t aCPU;
        CPU_ZERO(&aCPU);
        CPU_SET(2, &aCPU);
...
void WALKtoWHEEL_FUNCTION_FAST(void)
{
    ...
    if(pos_change_count == 2262){
        //JS
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[0] =100;//id
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[1] =1;//
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[2] =1;//ch
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_COMMAND= INIT_FIND_POS;

        FILE_LOG(logERROR) << "WHEEL HOME";
        MCJointFindHome(0, MC_GetID(RWH), MC_GetCH(RWH)+1);
        MCJointFindHome(1, MC_GetID(LWH), MC_GetCH(LWH)+1);
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[0] =4;//id
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[1] =1;//
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[2] =1;//ch
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_COMMAND= INIT_FIND_POS;
    }
//    if(pos_change_count == 2282){
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[0] =10;//id
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[1] =1;//
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[2] =1;//ch
//        sharedData->COMMAND[PODO_NO_DAEMON].USER_COMMAND= INIT_FIND_POS;
//    }
...
void WHEELup()
{
    ...
    if(pos_change_count == 5){
        joint->SetMoveJoint(RHP, -20., 3000,MOVE_ABSOLUTE);//c
        joint->SetMoveJoint(LHP, -20., 3000,MOVE_ABSOLUTE);//c
        joint->SetMoveJoint(RKN, 120,3000, MOVE_ABSOLUTE);//b//141
        joint->SetMoveJoint(LKN, 120,3000, MOVE_ABSOLUTE);//b
    }
    if(pos_change_count == 605){
        printf("Wheel up done!!!!!\n");
        FLAG_WHEELup = false;
        pos_change_count= 0;
    }
}
...
void WHEELdown()
{
    ...
    if(pos_change_count == 2){
        joint->SetMoveJoint(RHP, _POS_HIP_PITCH, 3000, MOVE_ABSOLUTE);// equilibrium point
        joint->SetMoveJoint(LHP, _POS_HIP_PITCH, 3000, MOVE_ABSOLUTE);// equilibrium point
        joint->SetMoveJoint(RKN, 145.0, 3000, MOVE_ABSOLUTE);// equilibrium point
        joint->SetMoveJoint(LKN, 145.0, 3000, MOVE_ABSOLUTE);// equilibrium point
    }
    if(pos_change_count == 605)
    {
        MCenableFrictionCompensation(0,4,2,DISABLE);
        ...
        MCBoardSetSwitchingMode(0,4,0);
        MCBoardSetSwitchingMode(1,10,0);
        MCJointGainOverride(0,4,2,0, 1000);
        MCJointGainOverride(1,10,2,0,1000);
    }
    if(pos_change_count == 805)
    {
        printf("Wheel down done!!!!!!!\n");
        FLAG_WHEELdown = false;
        pos_change_count = 0;
    }
}
...
void InitWheelHome(void){
    ...
    sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[0] =0;//id
    sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[1] =4;//id
    sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[2] =1;//ch
    sharedData->COMMAND[PODO_NO_DAEMON].USER_COMMAND= DAEMON_INIT_FIND_HOME;

    usleep(400*1000);
    sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[0] =0;//id
    sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[1] =10;//id
    sharedData->COMMAND[PODO_NO_DAEMON].USER_PARA_CHAR[2] =1;//ch
    sharedData->COMMAND[PODO_NO_DAEMON].USER_COMMAND= DAEMON_INIT_FIND_HOME;
    usleep(400*1000);
...
//    joint->SetMoveJoint(LF2, 0.0, pos_ms, MOVE_ABSOLUTE);
    ...
    joint->SetMoveJoint(RHP, -35.726, pos_ms, MOVE_ABSOLUTE);
    joint->SetMoveJoint(RKN, 65.139, pos_ms, MOVE_ABSOLUTE);
    joint->SetMoveJoint(RAP, -29.414, pos_ms, MOVE_ABSOLUTE);
    ...
    joint->SetMoveJoint(LHP, -35.726, pos_ms, MOVE_ABSOLUTE);
    joint->SetMoveJoint(LKN, 65.139, pos_ms, MOVE_ABSOLUTE);
    joint->SetMoveJoint(LAP, -29.414, pos_ms, MOVE_ABSOLUTE);
