...
void OpticalDisplacement::OD_MeasureState(){
    ...
    Mea_th = Mea_th + sharedData->IMU[0].YawVel*0.005*3.141592/180;
    Mea_pit = Mea_pit + sharedData->IMU[0].PitchVel*0.005*3.141592/180;

    // ================================================== //
    // Optical Flow Sensor...
    // ================================================== //
    int Opt1Xval = sharedData->OF.AccumX[0];
    int Opt1Yval = sharedData->OF.AccumY[0]; // right leg
    int Opt2Xval = sharedData->OF.AccumX[1]; // left leg
    int Opt2Yval = sharedData->OF.AccumY[1];
...
//        Mea_y = Mea_y + sin(Mea_th)*Robot_next_localX + cos(Mea_th)*Robot_next_localY;
    ...
    sharedData->OF.Xmea = (float)(Mea_x);
    sharedData->OF.Ymea = (float)(Mea_y);
    sharedData->OF.thmea = (float)(Mea_th);
    sharedData->OF.Pitmea = (float)(Mea_pit);
}
...
void OpticalDisplacement::OD_Zero(){
    ...
    int Opt1Yval = sharedData->OF.AccumX[0];
    int Opt2Yval = sharedData->OF.AccumX[1]; // left leg

    sharedData->OF.Xmea = 0;
    sharedData->OF.Ymea = 0;
    sharedData->OF.thmea = 0;
    sharedData->OF.Pitmea = 0;
